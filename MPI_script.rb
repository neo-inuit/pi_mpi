...
scheduler_command = [["mpirun.openmpi -mca btl ^openib -np %{nodes} --stdout %{output} %{prog} %{arg}", "oarstat %{jobid}", "oardel %{jobid}"], ["qsub -l nodes=%{nodes} -o %{output} %{pbs}", "qstat %{jobid}", "qdel %{jobid}"]]
...
def launch(hostname,login,cmd)
  begin
    ...
    if (!scheduler)
      ...
      #node reservation
      oar = ssh.exec!("oarsub -l /nodes=%{nodes}" % {nodes: $nodes})
    end
